...
const KpiModal = ({
  ...
  debug &&
    console.log(
      `### SK >>> MODAL - ${new Date().getTime()}: kpiMode: ${kpiMode}, kpi.name: ${
        kpi.name
      }, showModal: ${showModal}`
    );
  ...
  const [desc, setDesc] = useState(
    ['edit', 'delete'].includes(kpiMode) ? kpi.desc : ''
  );
  ...
  useEffect(() => {
    debug &&
      console.log(`### SK >>> MODAL - ${new Date().getTime()}: useEffect(kpi)`);
    setName(kpi.name);
    setDesc(kpi.desc);
  ...
  const refreshPreview = useCallback(async (res) => {
    if (!res.query) {
      ...
      const result = await utils.runQuery([
        { index: 0, accountIds: [res.accountId], nrqlQuery: res.query },
      ]);
  ...
  debug &&
    console.log(
      `### SK >>> MODAL - ${new Date().getTime()}: kpiMode: ${kpiMode}, kpi.name: ${
        kpi.name
      }, showModal: ${showModal}, previewEnabled: ${previewEnabled}`
    );
  return (
    <div>
      <Modal
                ...
                <div className="modal-component-nrql-editor">
                  <NrqlEditor
                    ...
                    onSave={(res) => {
                      ...
                      debug &&
                        console.log('Preview button clicked - res: ', res);
                      refreshPreview(res);
                    }}
                  />
                </div>
          ...
          <div>
            <Button
              ...
              onClick={(e) => {
                debug &&
                  console.log(
                    `### SK >>> MODAL - ${new Date().getTime()}: SAVE button cliecked - "${
                      kpiMode === 'add'
                        ? 'Add'
                        : kpiMode === 'delete'
                        ? 'Delete'
            ...
            {kpiMode === 'delete' && (
              <Button
                ...
                onClick={(e) => {
                  debug &&
                    console.log(
                      `### SK >>> MODAL - ${new Date().getTime()}: CANCEL button cliecked -- event: `,
                      e
                    );
                  setShowModal(false);
                }}
